...
### 0. 影片預處理 (重要)
為了順利使用 `-c copy` 以節省 CPU 資源，MP4 格式需符合 YouTube 串流標準。若原始影片格式不符，請先在本地執行以下指令進行轉碼：

```bash
# 將 video.mp4 轉碼為符合標準的 v.mp4
ffmpeg -i v.mp4 -c:v libx264 -preset fast -b:v 3000k -maxrate 3000k -bufsize 6000k -pix_fmt yuv420p -c:a aac -b:a 96k -ar 44100 -g 60 video.mp4
```
